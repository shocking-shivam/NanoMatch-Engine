...
void print_comparison_table(const BenchStats& baseline,
                            const BenchStats& slab,
                            const BenchStats& slabLogged) {
    const auto latency_speedup = [&](double base, double opt) {
        return base / opt;
    };
    const auto throughput_speedup = [&](double base, double opt) {
        return opt / base;
    };
    ...
    const auto row = [&](const char* label, double b, double s, double sl) {
        std::cout << std::setw(18) << label
                  ...
                  << std::setw(18) << s
                  << std::setw(18) << sl
                  << std::setw(14) << latency_speedup(b, s) << "x"
                  << std::setw(14) << (sl / s) << "x\n";
    };
    ...
    std::cout << std::setw(18) << "throughput (M/s)"
              << std::setw(18) << throughput_mps(baseline)
              << std::setw(18) << throughput_mps(slab)
              << std::setw(18) << throughput_mps(slabLogged)
              << std::setw(14) << throughput_speedup(throughput_mps(baseline), throughput_mps(slab)) << "x"
              << std::setw(14) << (throughput_mps(slabLogged) / throughput_mps(slab)) << "x\n";
